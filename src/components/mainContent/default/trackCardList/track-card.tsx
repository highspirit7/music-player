...
function TrackCard({ data, onNewTrackClick }: { data: Track; onNewTrackClick: () => void }) {
  ...
  return (
    <Card key={data.id} className="flex-shrink-0 w-48">
      <CardContent className="p-4">
        <div className="aspect-square relative mb-3  group">
          ...
          {currentPlayingTrack.id === data.id ? (
            <div className="absolute inset-0 bg-black bg-opacity-50 grid grid-cols-3 grid-rows-3 rounded">
              {playlists.length > 0 && (
                <SaveToPlaylistDialog isOnCard={true} track={data} playlists={playlists} />
              )}
              {isPlaying ? (
                <Pause
        ...
        <p className="text-sm text-muted-foreground line-clamp-1">{data.artist_name}</p>
      </CardContent>
    </Card>
  )
}
