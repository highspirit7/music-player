...
import { createPlayingTrack, getNextIndex, getPrevIndex, toMinutesAndSeconds } from '@/lib/utils'
...
function BottomControlBar() {
  ...
  const handleSkipClick = (type: 'back' | 'forward') => {
    ...
    if (newTrackIndex >= 0) {
      setCurrentPlayingTrack(createPlayingTrack(currentPlayingList[newTrackIndex], newTrackIndex))
  ...
  useEffect(() => {
    if (howlInstance) {
      howlInstance.on('end', () => {
        ...
        const nextTrackIndex = getNextIndex(currentPlayingList, currentPlayingTrack.index)
        setCurrentPlayTime(0)
        ...
        <div className="grid grid-cols-3 items-center">
          ...
          <div className="flex flex-col items-center space-y-2 max-w-md">
            <div className="flex items-center space-x-4">
              <Button size="icon" variant="ghost" onClick={() => handleSkipClick('back')}>
              ...
              <Button size="icon" variant="ghost" onClick={() => handleSkipClick('forward')}>
